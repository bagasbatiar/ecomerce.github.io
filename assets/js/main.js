...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Ambil 4 produk sebagai "Best Seller" (misal produk 1, 4, 7, dan 11)
    const displayIds = [1, 4, 7, 11];
    const bestSellers = PRODUCTS.filter(p => displayIds.includes(p.id));

    container.innerHTML = ""; // Bersihkan isi manual sebelumnya
    ...
    bestSellers.forEach(product => {
      ...
      product.variants.forEach((v, index) => {
        swatchesHTML += `<span class="swatch ${index === 0 ? 'active' : ''}" style="background-color: ${v.colorCode};"></span>`;
      });

      // Template HTML (Sesuai style asli Anda)
      const html = `
          <div class="col-lg-3 col-md-6">
            <div class="product-item">
              <div class="product-image">
                <div class="product-badge">${product.id === 11 ? 'Stok Terbatas' : 'Lagi Tren'}</div>
                <img src="${mainImg}" alt="${product.name}" class="img-fluid" loading="lazy">
                <a href="product-details.html?id=${product.id}" class="cart-btn text-center text-decoration-none">Lihat Detail</a>
              </div>
              <div class="product-info">
                <div class="product-category">${product.category}</div>
                <h4 class="product-name"><a href="product-details.html?id=${product.id}">${product.name}</a></h4>
